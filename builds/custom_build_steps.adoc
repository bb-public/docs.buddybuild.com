...
- <<prebuild,immediately before the build, after dependencies are
  resolved>>
- <<postbuild,immediately after the build completes>>
...
The post-build script runs after the build.
...
Typically, you would use this script to perform any required operations,
whether your build, test execution, or deployment was successful or not.
One example would be to fetch test output that buddybuild does not
currently parse and present within the dashboard.
======
