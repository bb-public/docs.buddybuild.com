...
grep "ERROR" *.logs
scp *.logs user@remote:failures/
----

This example searches for `ERROR` in log files. If there are no error
messages, `grep` returns a non-zero status and the script terminates. If
there are errors, the `scp` command copies the log files to a remote
host for further inspection.
